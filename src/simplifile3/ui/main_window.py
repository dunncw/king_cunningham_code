# simplifile3/ui/main_window.py - Main UI window for simplifile3
...
class Simplifile3MainWindow(QWidget):
    """Main Simplifile3 UI window with workflow-first design"""
    ...
    def init_ui(self):
        ...
        # Workflow Selection (no county selector)
        self.workflow_selector = WorkflowSelectorWidget()
        self.workflow_selector.selection_changed.connect(self.on_workflow_selection_changed)
        ...
        # Initialize file inputs with current workflow selection
        self.initialize_file_inputs()
    
    def initialize_file_inputs(self):
        """Initialize file inputs based on current workflow selection"""
        if self.workflow_selector.is_selection_valid():
            workflow_config = self.workflow_selector.get_workflow_config()
            if workflow_config:
                self.current_workflow_config = workflow_config
                self.file_inputs.update_for_workflow(workflow_config)
                
                if hasattr(self, 'output_text'):
                    workflow_name = self.workflow_selector.get_workflow_name()
                    self.log_output(f"Selected: {workflow_name}")
                    
                    input_type = workflow_config.get('input_type', 'unknown')
                    self.log_output(f"Input type: {input_type}")
    
    def on_workflow_selection_changed(self, workflow_id: str, workflow_config: Dict[str, Any]):
    ...
    def validate_basic_inputs(self) -> List[str]:
        ...
        if not self.workflow_selector.is_selection_valid():
            errors.append("Workflow selection required")
    ...
    def log_validation_header(self):
        ...
        self.log_output(f"Workflow: {self.workflow_selector.get_workflow_name()}")
