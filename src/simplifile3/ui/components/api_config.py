# simplifile3/ui/components/api_config.py - API configuration widget (copied from simplifile2)
...
import os
import json
from simplifile2.ui.components.api_config import APIConfigWidget

class DeedbackMainWindow(QWidget):
    def __init__(self):
        super().__init__()
        # Use the same API config widget
        self.api_config = APIConfigWidget()
        # ... rest of initialization
    
    def get_api_token(self) -> str:
        """Get API token from shared config"""
        return self.api_config.get_api_token()
...
class APIConfigWidget(QGroupBox):
    ...
    def __init__(self):
        ...
        self.config_file = os.path.join(os.path.expanduser("~"), ".simplifile3_conf.json")
        self.config = {}
        self.init_ui()
        self.load_config()
